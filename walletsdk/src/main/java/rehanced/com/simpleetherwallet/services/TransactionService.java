...
import rehanced.com.simpleetherwallet.bean.TransactionReceipt;
...
import static rehanced.com.simpleetherwallet.network.EtherscanAPI.CHAIN_ID;
import static rehanced.com.simpleetherwallet.receiver.TransactionStatusReceiver.REQUEST_UUID;
import static rehanced.com.simpleetherwallet.receiver.TransactionStatusReceiver.TX_HASH;
import static rehanced.com.simpleetherwallet.receiver.TransactionStatusReceiver.TX_ACTION;
import static rehanced.com.simpleetherwallet.receiver.TransactionStatusReceiver.TX_STATUS;
import static rehanced.com.simpleetherwallet.receiver.TransactionStatusReceiver.TX_STATUS_FAIL;
import static rehanced.com.simpleetherwallet.receiver.TransactionStatusReceiver.TX_STATUS_BUILD;
import static rehanced.com.simpleetherwallet.receiver.TransactionStatusReceiver.TX_STATUS_SUCCESS;
import static rehanced.com.simpleetherwallet.utils.RawTransactionUtils.getTransaction;
...
public class TransactionService extends IntentService {
            ...
            EtherscanAPI.getInstance().getNonceForAddress(fromAddress, new Callback() {
                ...
                public void onResponse(Call call, final Response response) throws IOException {
                    try {
                        ...
                        RawTransaction tx = getTransaction(nonce, contractAddress, amount, gas_price, gas_limit, data, toAddress);
        ...
        EtherscanAPI.getInstance().forwardTransaction("0x" + Hex.toHexString(signed), new Callback() {
            ...
            public void onResponse(Call call, final Response response) throws IOException {
                ...
                try {
                   ...
                   startBroadcast(hash, requestUuid, TX_STATUS_SUCCESS);
    ...
    private void suc(String hash, String requestUuid) {
        MyLog.i("txHash" + hash);
        startBroadcast(hash, requestUuid, TX_STATUS_BUILD);
        builder
...
//        builder.setContentIntent(contentIntent);
    ...
    private void startBroadcast(String hash, String requestUuid, String status) {
        ...
        intent.setAction(TX_ACTION);
        if (hash != null) {
            intent.putExtra(TX_HASH, hash);
        }
        intent.putExtra(REQUEST_UUID, requestUuid);
        intent.putExtra(TX_STATUS, status);
    ...
    private void error(String err, String requestUuid) {
        if(requestUuid!=null)
        startBroadcast(null, requestUuid, TX_STATUS_FAIL);
        builder
